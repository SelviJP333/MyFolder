...
const SynchUpdateData = ({aTable, aData, setaTable, setaData}) => {
  useEffect(() => {
    if (aTable.length > 0) {
      update();
    }
        ...
        db.transaction(txn => {
          txn.executeSql(
            ...
            async function (tx, res) {
              ...
              aData.map(item => {
                txn.executeSql(
                  `UPDATE DeviceSynchDataLog set IsObsolete = ?,SynchedDateTime=?,SynchMessage=?  where RowID=? and Action='U' and IsObsolete=0 and DatalogID=?`,
                  [
                    1,
                    response.data.response.currentSynchDateTimeDevice,
                    'Complete',
...
export default SynchUpdateData;
