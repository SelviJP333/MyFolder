...
let deletephaseid;
let deleteCompanyid;
const EditTimeSheetScreen = ({route, navigation}) => {
...
and tj.JobID=? and tc.CompanyId=?`,
  ...
  function showDatePicker(job_i, company_i, e, ss, job) {
    var date = job[0].split('*#^*');
    const lastElement = date[date.length - 1];
    console.log('lastElement', lastElement);
    if (lastElement != '' && lastElement != 'null') {
      setSelectedDate(new Date(lastElement));
    }
  ...
  const GetWholeData = async newArray => {
    ...
    db.transaction(tx => {
      tx.executeSql(
        `select ts.StatusName, u.EmployeeId,u.FullName,tc.CompanyID ,lc.CompanyName ,tj.JobID,lj.JobNo ,lj.JobName,tp.* ,tj.EstProjectCompletionDate, lp.PhaseCodeName ,lp.PhaseCode from Timesheet t
        left join TimesheetEmployee te on t.TimesheetID=te.TimesheetID
        left join TimesheetCompany tc on te.TimesheetEmployeeID = tc.TimesheetEmployeeID
        ...
        async (tx, res) => {
          ...
          for (let j = 0; j < newArray.length; j++) {
            ...
            result.sort(function (a, b) {
              let res1 = parseInt(a[0][0].split('*#^*')[0]);
              let res2 = parseInt(b[0][0].split('*#^*')[0]);
              return res1 - res2;
            });
  ...
  const GetJobs = (selectedCompany, company_index = null, type) => {
    ...
    if (type === 'View All') {
      let q = `select JobID , JobName , JobNo from (select * from LkpJob 
      ...
      AND [IsRestricted] = 1 AND	PhaseCodeID <> (SELECT [PhaseGroup] FROM [LkpCompany] where CompanyId = ((SELECT companyid FROM [LkpJob] where JobID = ${SetSelectedJob} ))  )
       ...
       `;
        ...
        tx.executeSql(query, [], (tx, res) => {
          ...
          //   return !WholeData.some(function (o2) {
          //     if (UserID[indexvalue] == o2.EmployeeID) {
          //       return o2.PhaseCodeID === o1.PhaseCodeID; // assumes unique id
          //     }
          //   });
          // });
          let newArray = temp.map(item => {
  ...
  const deleteCompany = (e, company_index, com) => {
    ...
    // let phases = [];
    // if (com[1] && com[1].length !== 0) {
    //   if (com[1][0][1] != undefined) {
    //     setdeletetime(phases);
    //   }
    // }
    setcompany_index(company_index);
  ...
  const GetpickerDate = () => {
    let datepickerTime = new Date().toLocaleDateString('en-US');
  ...
  const handleAddRow = async (
    ...
    let SelectedTimesheetEmployeeID = ExistingEmployee.filter(
      item => item.value == SelectedEmployeeId.toString(),
    ).map(item => item.TimesheetEmployeeID);

    let SelectedTimesheetCompanyID = ExistingCompany.filter(
      item =>
        item.ID == selectcom &&
        item.NewEmployeeID == SelectedEmployeeId.toString(),
    ...
    if (modalComapanyVisible === true && deleteComapany_modal !== true) {
      ...
      selectedCompany.forEach((e, i) => {
        ...
        let compdata = 0;
        while (
          compdata < TableData.length &&
          number > parseInt(TableData[compdata][0][0].match(/(\d+)\*\#\^\*/)[1])
        ) {
          compdata++;
        }
      ...
      db.transaction(txn => {
        for (let i = 0; i < selectetJobID.length; i++) {
          ...
          var DateTime = CurrentUTCTime();
          var TimesheetID = generateGuidQuickly();
          var datepickerTime = GetpickerDate();
          txn.executeSql(
            ...
            [
              ...
              selectetJobID[i],
              HasEstimatedProjectCompletionDate === 1 ? datepickerTime : null,
              SubmitUserID,
              DateTime,
            ],
            function (tx, res) {
...
`;
      ...
      let removed = Company.map(e => {
        if (e.value.split('*#^*').pop().trim() === selectedcompanyname.trim()) {
...
`;
            ...
            let changephase = e[1].map((j_data, j_index) => {
              if (j_index === phasejob_index) {
                if (j_data.length > 1) {
                  ...
                  modification.sort(function (a, b) {
                    return (
                      Number(a[0].split('*#^*')[0]) -
                      Number(b[0].split('*#^*')[0])
                    );
  ...
  const get_set = (
    ...
    let pass_company_data = e[0][0].split('*')[0];
    if (pass_company_data.includes('+')) {
      pass_com = pass_company_data.split('-')[0];
    } else {
      pass_com = pass_company_data;
    }
  ...
  const RemoveEmploye = () => {
    ...
    let timesheetEmployeeiddata = null;
    db.transaction(txn => {
      txn.executeSql(
        `SELECT TimesheetEmployeeID FROM TimesheetEmployee where TimesheetID = ? and EmployeeId = ?`,
        [route.params.item.TimesheetID, UserID[indexvalue]],
        async function (tx, res) {
          for (let i = 0; i < res.rows.length; i++) {
            timesheetEmployeeiddata = res.rows.item(i).TimesheetEmployeeID;
          }
        },
      );
    });
    ...
    db.transaction(txn => {
      ...
      let query = `
      INSERT INTO DeviceSynchDataLog ( Action, TableName,RowID, ColumnName, ColumnValue,DateTime,SourceDeviceID,IsObsolete,SynchedDateTime,SynchMessage)
      select  'D' as Action,'TimesheetEmployee' as TableName,TimesheetEmployeeID as RowID,'-' as ColumnName,''as ColumnValue,'${dateTime}' as DateTime,'${Deviceid}' as  SourceDeviceID,'${0}' as IsObsolete,'${0}' as  SynchedDateTime,'${'-'}' as SynchMessage from  TimesheetEmployee where TimesheetEmployeeID = '${timesheetEmployeeiddata}'
  UNION
  select  'D' as Action,'TimesheetCompany' as TableName, tc.TimesheetCompanyID as RowID,'-' as ColumnName,''as ColumnValue,'${dateTime}' as DateTime,'${Deviceid}' as  SourceDeviceID,'${0}' as IsObsolete,'${0}' as  SynchedDateTime,'${'-'}' as SynchMessage from  TimesheetCompany tc LEFT JOIN TimesheetEmployee te on te.TimesheetEmployeeID = tc.TimesheetEmployeeID WHERE te.TimesheetEmployeeID = '${timesheetEmployeeiddata}'
  UNION
  select  'D' as Action,'TimesheetJob' as TableName, tj.TimesheetJobID as RowID,'-' as ColumnName,''as ColumnValue,'${dateTime}' as DateTime,'${Deviceid}' as  SourceDeviceID,'${0}' as IsObsolete,'${0}' as  SynchedDateTime,'${'-'}' as SynchMessage from  TimesheetJob tj LEFT JOIN TimesheetCompany tc on tj.TimesheetCompanyID = tc.TimesheetCompanyID LEFT JOIN TimesheetEmployee te on te.TimesheetEmployeeID = tc.TimesheetEmployeeID WHERE te.TimesheetEmployeeID = '${timesheetEmployeeiddata}'
  UNION
  select  'D' as Action,'TimesheetPhaseCode' as TableName, tp.TimesheetPhaseCodeID as RowID,'-' as ColumnName,''as ColumnValue,'${dateTime}' as DateTime,'${Deviceid}' as  SourceDeviceID,'${0}' as IsObsolete,'${0}' as  SynchedDateTime,'${'-'}' as SynchMessage from 
  TimesheetPhaseCode tp LEFT JOIN  TimesheetJob tj on tp.TimesheetJobID=tj.TimesheetJobID 
  LEFT JOIN TimesheetCompany tc on tj.TimesheetCompanyID = tc.TimesheetCompanyID LEFT JOIN TimesheetEmployee te on te.TimesheetEmployeeID = tc.TimesheetEmployeeID WHERE te.TimesheetEmployeeID = '${timesheetEmployeeiddata}'
      `;
      txn.executeSql(query, []);
    });
  ...
  const GetExistingEmployee = async () => {
    db.transaction(tx => {
      tx.executeSql(
        `select te.TimesheetEmployeeID,e.FullName ,te.TimesheetID,te.EmployeeId,(select Username from User where EmployeeId=te.EmployeeId) as Username   from TimesheetEmployee te

        left join Employee e on e.EmployeeId = te.EmployeeId where te.TimesheetID=?`,
      ...
      <SafeAreaView style={{flex: 1}}>
        ...
        {isCopyModalVisible ? (
          <TimeSheet_Copypop
            ...
            EditTimesheetID={route.params.item.TimesheetID}
            close=""
            ApproveUsernameList=""
          />
        ) : null}
        ...
        <View style={styles.header}>
          ...
          <View style={styles.btncontainer}>
            <TouchableOpacity style={styles.leftButton} onPress={Save}>
              <Text style={styles.backbtn}>&#60; Back to all Timesheets</Text>
            </TouchableOpacity>

            <Pressable onPress={GetEmployee} style={styles.rightButton}>
